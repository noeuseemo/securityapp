...
{
    public partial class PhysicalClientsForm : Form
    {
        ...
        public PhysicalClientsForm()
        {
            InitializeComponents();
            this.Text = "Клиенты (физические лица)";
            ...
            try
            {
                using (var connection = DatabaseHelper.GetConnection())
                {
                    ...
                    var command = new SqlCommand(
                        @"SELECT c.Id, c.LastName, c.FirstName, c.MiddleName, c.Address, c.PassportData,
        ...
        {
            var form = new PhysicalClientEditForm(null);
        ...
        private void BtnEdit_Click(object sender, EventArgs e)
        {
            ...
            var form = new PhysicalClientEditForm(id);
            if (form.ShowDialog() == DialogResult.OK)
            {
                LoadClients();
            }
                              ...
                              MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    using (var connection = DatabaseHelper.GetConnection())
                    {
                        connection.Open();
                        var command = new SqlCommand(
                            "DELETE FROM PhysicalClients WHERE Id = @Id", connection);
        ...
        private void BtnContracts_Click(object sender, EventArgs e)
        {
            ...
            var clientName = $"{dataGridView.SelectedRows[0].Cells["LastName"].ToString()} " +
                            $"{dataGridView.SelectedRows[0].Cells["FirstName"].ToString()}";
            var form = new ContractsForm(clientId, "Physical", clientName);
            form.ShowDialog();
        }
    }
}